...
if (!isLoggedIn()) {
    renderGuestbookLockView(
      ...
      'The digital catalog is available exclusively to registered visitors. Sign our Digital Guestbook to gain full access to all types of artifacts and collections.'
    );
    return;
}
...
<div class="wrap">
  <form method="GET" action="index.php" class="search-row" style="max-width:500px;margin:0 auto 44px">
    ...
    <div class="s-wrap">
      ...
      <input type="text" name="q" class="s-inp" placeholder="Search types of artifacts..." value="<?= htmlspecialchars($search) ?>" oninput="liveSearch('liveSearchDept','cat-card')">
    </div>
    <button type="submit" class="btn-srch artifact-search-icon-btn" aria-label="Search types of artifacts">
  ...
  <?php if (empty($cats)): ?>
    <div class="empty-state"><div class="ei">&#127963;</div><h3>No types of artifacts found</h3><p>Try a different search term.</p></div>
  <?php else: ?>
